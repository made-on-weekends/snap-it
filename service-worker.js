...
// ========== FULL PAGE CAPTURE (SCROLL & STITCH VIA CDP + OFFSCREEN CANVAS) ==========

const FULLPAGE_DETECT_SCRIPT = `
  (() => {
    const docElem = document.scrollingElement || document.documentElement || document.body;
    const winHeight = window.innerHeight || 800;
    const winWidth = window.innerWidth || 1200;

    // Convert lazy images to eager to trigger network loading
    try {
    ...
    // Inject temporary scrollbar hiding style
    let scrollbarStyle = document.getElementById('__snapit_hide_scrollbars__');
    if (!scrollbarStyle) {
      scrollbarStyle = document.createElement('style');
      scrollbarStyle.id = '__snapit_hide_scrollbars__';
      scrollbarStyle.textContent = '::-webkit-scrollbar { display: none !important; } * { scrollbar-width: none !important; }';
      document.head.appendChild(scrollbarStyle);
    }

    // 1. Check if the window/document itself is scrollable
    if (docElem && docElem.scrollHeight > winHeight + 30) {
      return {
        isWindow: true,
        totalWidth: Math.max(docElem.scrollWidth, winWidth),
        totalHeight: docElem.scrollHeight,
    ...
    // 2. Search for scrollable containers (common in SPAs like Upwork, React apps, dashboards)
    let bestEl = null;
    let maxScroll = winHeight;

    const candidates = document.querySelectorAll('main, [role="main"], #main-content, #root, #app, .up-layout, [class*="layout"], [class*="dashboard"], [class*="content"], [class*="container"], [class*="scroll"], body > div');
    for (const el of candidates) {
      const style = window.getComputedStyle(el);
      const isScrollable = (style.overflowY === 'auto' || style.overflowY === 'scroll') && el.scrollHeight > maxScroll;
      if (isScrollable) {
    ...
    if (!bestEl) {
      ...
      for (const el of all) {
        if (el.scrollHeight > maxScroll && el.scrollHeight > winHeight + 30) {
          const style = window.getComputedStyle(el);
    ...
    if (bestEl) {
      bestEl.dataset.snapitScrollTarget = 'true';
      return {
        isWindow: false,
        totalWidth: Math.max(bestEl.scrollWidth, bestEl.clientWidth, winWidth),
        totalHeight: bestEl.scrollHeight,
        viewportWidth: bestEl.clientWidth || winWidth,
        viewportHeight: bestEl.clientHeight || winHeight,
        originalScrollX: bestEl.scrollLeft || 0,
        originalScrollY: bestEl.scrollTop || 0,
      };
    }

    // 3. Fallback: single viewport
    return {
      isWindow: true,
      totalWidth: docElem ? docElem.scrollWidth : winWidth,
      totalHeight: docElem ? docElem.scrollHeight : winHeight,
      viewportWidth: winWidth,
      viewportHeight: winHeight,
      originalScrollX: window.scrollX || window.pageXOffset || 0,
      originalScrollY: window.scrollY || window.pageYOffset || 0,
...
function getScrollSliceScript(scrollY, isFirstSlice) {
  return `
    (() => {
      const isFirst = ${isFirstSlice ? 'true' : 'false'};
      const targetY = ${scrollY};

      const targetEl = document.querySelector('[data-snapit-scroll-target="true"]');
      if (targetEl) {
        targetEl.scrollTop = targetY;
      } else {
        window.scrollTo({ top: targetY, left: 0, behavior: 'instant' });
      }

      // Hide fixed/sticky elements on slices > 0 so headers do not duplicate across the page
      if (!window.__snapitHiddenElements) {
        window.__snapitHiddenElements = [];
      }

      if (!isFirst) {
        if (window.__snapitHiddenElements.length === 0) {
          document.querySelectorAll('*').forEach(el => {
            const style = window.getComputedStyle(el);
            if (style.position === 'fixed' || style.position === 'sticky') {
              window.__snapitHiddenElements.push({ el, origVisibility: el.style.visibility });
              el.style.visibility = 'hidden';
            }
          });
        }
      } else {
        // First slice: keep visible
        if (window.__snapitHiddenElements.length > 0) {
          window.__snapitHiddenElements.forEach(({ el, origVisibility }) => {
...
function getCleanupScript(origX, origY) {
  return `
    (() => {
      // Remove temporary scrollbar style
      const scrollbarStyle = document.getElementById('__snapit_hide_scrollbars__');
      if (scrollbarStyle) scrollbarStyle.remove();

      // Restore fixed/sticky elements
      if (window.__snapitHiddenElements && window.__snapitHiddenElements.length > 0) {
        window.__snapitHiddenElements.forEach(({ el, origVisibility }) => {
          el.style.visibility = origVisibility;
        });
        window.__snapitHiddenElements = [];
      ...
      const targetEl = document.querySelector('[data-snapit-scroll-target="true"]');
      if (targetEl) {
        targetEl.scrollTop = ${origY || 0};
        targetEl.scrollLeft = ${origX || 0};
        delete targetEl.dataset.snapitScrollTarget;
      } else {
        window.scrollTo({ top: ${origY || 0}, left: ${origX || 0}, behavior: 'instant' });
      }
    })()
  `;
}
...
async function captureFullPage(tabId, settings) {
  ...
  try {
    ...
    console.warn('chrome.debugger.attach failed, attempting visible tab fallback if active:', attachErr);
  }

  // Execute in tab helper (CDP Runtime.evaluate or chrome.scripting fallback)
  const execScript = async (expression) => {
    if (debuggerAttached) {
      const evalRes = await sendDebuggerCommand(tabId, 'Runtime.evaluate', {
        expression,
        awaitPromise: true,
        returnByValue: true,
      });
    ...
    throw new Error('Unable to execute script in tab (debugger and scripting unavailable).');
  };

  // Viewport screenshot capture helper
  const captureSlice = async () => {
    if (debuggerAttached) {
      const snapRes = await sendDebuggerCommand(tabId, 'Page.captureScreenshot', {
        format: settings.format === 'jpeg' ? 'jpeg' : 'png',
        quality: settings.format === 'jpeg' ? settings.quality : undefined,
        captureBeyondViewport: false,
      });
      return `data:${mimeType};base64,${snapRes.data}`;
    } else {
      const tab = await chrome.tabs.get(tabId);
      const format = settings.format === 'jpeg' ? 'jpeg' : 'png';
      const options = { format };
      if (format === 'jpeg') options.quality = settings.quality;
      return new Promise((resolve, reject) => {
        chrome.tabs.captureVisibleTab(tab.windowId, options, (res) => {
          if (chrome.runtime.lastError) reject(new Error(chrome.runtime.lastError.message));
          else if (!res) reject(new Error('Failed to capture visible tab slice'));
          else resolve(res);
        });
      });
    }
  ...
  try {
    // 1. Detect layout and scrollable target (window vs inner element like Upwork)
    layout = await execScript(FULLPAGE_DETECT_SCRIPT);

    if (!layout || !layout.viewportHeight || layout.viewportHeight <= 0) {
      throw new Error('Could not detect page dimensions for full-page screenshot.');
    }

    const maxDimension = 16384;
    const totalHeight = Math.min(layout.totalHeight, maxDimension);
    const viewportHeight = layout.viewportHeight;
    const viewportWidth = layout.viewportWidth;

    // Single screen / non-scrollable page
    if (totalHeight <= viewportHeight + 15) {
      return await captureSlice();
    }

    // 2. Compute slice positions and scroll-capture each slice
    const capturedSlices = [];
    const maxSlices = 40;
    let currentY = 0;
    let isLast = false;

    while (currentY < totalHeight && capturedSlices.length < maxSlices) {
      const isFirst = capturedSlices.length === 0;

      // Scroll to slice position & hide fixed elements on slices > 0
      await execScript(getScrollSliceScript(currentY, isFirst));

      // Wait for reflow / lazy loading
      await sleep(150);

      // Capture visible slice
      const sliceDataUrl = await captureSlice();
      const blob = await dataUrlToBlob(sliceDataUrl);
      const bitmap = await createImageBitmap(blob);
      capturedSlices.push({ bitmap, scrollY: currentY, isLast });
      ...
      if (currentY + viewportHeight >= totalHeight) {
        break;
      } else if (currentY + viewportHeight * 2 > totalHeight) {
        // Next slice is the final slice
    ...
    // 3. Clean up and restore tab state
    try {
      await execScript(getCleanupScript(layout.originalScrollX, layout.originalScrollY));
    } catch {}

    if (capturedSlices.length === 0) {
      throw new Error('Failed to capture screenshot slices.');
    }

    // 4. Stitch slices on OffscreenCanvas
    const scale = capturedSlices[0].bitmap.width / viewportWidth;
    const canvasWidth = capturedSlices[0].bitmap.width;
    const canvasHeight = Math.min(Math.round(totalHeight * scale), maxDimension);
    ...
    for (const slice of capturedSlices) {
      const destY = Math.round(slice.scrollY * scale);
      ctx.drawImage(slice.bitmap, 0, destY);
      slice.bitmap.close();
    }

    const quality = settings.format === 'jpeg' ? (settings.quality || 85) / 100 : undefined;
