...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const themeRadio = document.querySelector(
    `input[name="theme"][value="${settings.theme ?? DEFAULT_SETTINGS.theme}"]`
  );
...
document.querySelectorAll('input[name="theme"]').forEach(radio => {
  radio.addEventListener('change', async () => {
    ...
    try {
      ...
      await chrome.storage.sync.set({ settings: { ...settings, theme: selected } });
    } catch (e) {
...
// Sync theme radio buttons if theme changes from another window or toggle
if (typeof chrome !== 'undefined' && chrome.storage?.onChanged) {
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area === 'sync' && changes.settings?.newValue?.theme !== undefined) {
